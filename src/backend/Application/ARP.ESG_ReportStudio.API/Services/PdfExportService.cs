...
public sealed class PdfExportService : IPdfExportService
{
    ...
    public PdfExportService(ILocalizationService localizationService)
    {
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    ...
                    {
                        column.Item().PageBreak();
                        column.Item().Element(c => ComposeTableOfContents(c, report, options.Language));
                    }
                    ...
                    {
                        column.Item().PageBreak();
                        column.Item().Element(c => ComposeReportSections(c, report, options.Language));
                    }
    ...
    private void ComposeTitlePage(IContainer container, GeneratedReport report, PdfExportOptions options)
    {
        container.Column(column =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(options.VariantName))
            {
                ...
                column.Item().AlignCenter().Text($"{variantLabel} {options.VariantName}")
                    .FontSize(14).Italic();
            }
            ...
            if (!string.IsNullOrWhiteSpace(options.Language) && options.Language != "en-US")
            {
                ...
                column.Item().AlignCenter().Text($"{languageLabel} {options.Language}")
                    .FontSize(14).Italic();
            }
            ...
            // Report metadata
            var reportModeLabel = _localizationService.GetLabel("label.report-mode", options.Language);
            var reportScopeLabel = _localizationService.GetLabel("label.report-scope", options.Language);
            var generatedByLabel = _localizationService.GetLabel("label.generated-by", options.Language);
            var generatedAtLabel = _localizationService.GetLabel("label.generated-at", options.Language);
            var totalSectionsLabel = _localizationService.GetLabel("label.total-sections", options.Language);
            ...
            column.Item().Table(table =>
            {
                ...
                table.Cell().Text(reportModeLabel).SemiBold();
                table.Cell().Text(report.Period.ReportingMode);
                
                table.Cell().Text(reportScopeLabel).SemiBold();
                table.Cell().Text(report.Period.ReportScope);
                
                table.Cell().Text(generatedByLabel).SemiBold();
                table.Cell().Text(report.GeneratedByName);
                
                table.Cell().Text(generatedAtLabel).SemiBold();
                table.Cell().Text(_localizationService.FormatDate(report.GeneratedAt, options.Language));
    ...
    private void ComposeTableOfContents(IContainer container, GeneratedReport report, string? language = null)
    {
        container.Column(column =>
        {
            var tocLabel = _localizationService.GetLabel("label.table-of-contents", language);
            column.Item().Text(tocLabel).FontSize(18).Bold();
    ...
    private void ComposeReportSections(IContainer container, GeneratedReport report, string? language = null)
    {
        container.Column(column =>
        {
            foreach (var section in report.Sections.OrderBy(s => s.Section.Order))
            {
                column.Item().Element(c => ComposeSection(c, section, language));
                column.Item().PaddingBottom(20);
            }
        });
    }

    private void ComposeSection(IContainer container, GeneratedReportSection section, string? language = null)
    {
        container.Column(column =>
        {
            // Section header
            column.Item().PaddingBottom(10).Column(headerColumn =>
            {
                ...
                {
                    row.AutoItem().Text($"{categoryLabel} {section.Section.Category}").FontSize(9);
                    row.AutoItem().PaddingLeft(15).Text($"{ownerLabel} {section.Owner?.Name ?? "Unassigned"}").FontSize(9);
                    row.AutoItem().PaddingLeft(15).Text($"{statusLabel} {section.Section.Status}").FontSize(9);
                });
            });
            ...
            {
                column.Item().Element(c => ComposeDataPoints(c, section.DataPoints, language));
            }
            
            // Assumptions
            if (section.Assumptions.Count > 0)
            {
                column.Item().PaddingTop(10).Element(c => ComposeAssumptions(c, section.Assumptions, language));
            }
            
            // Gaps
            if (section.Gaps.Count > 0)
            {
                column.Item().PaddingTop(10).Element(c => ComposeGaps(c, section.Gaps, language));
            }
        });
    }

    private void ComposeDataPoints(IContainer container, List<DataPointSnapshot> dataPoints, string? language = null)
    {
        container.Table(table =>
        {
            ...
            table.Header(header =>
            {
                header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text(fieldLabel).SemiBold();
                header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text(valueLabel).SemiBold();
                header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text(unitLabel).SemiBold();
                header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text(statusLabel).SemiBold();
            });
    ...
    private void ComposeAssumptions(IContainer container, List<AssumptionRecord> assumptions, string? language = null)
    {
        container.Column(column =>
        {
            column.Item().Background(Colors.Orange.Lighten4).Padding(8).Column(assumptionColumn =>
            {
                ...
                assumptionColumn.Item().Text(assumptionsLabel).FontSize(12).SemiBold();
                ...
                foreach (var assumption in assumptions)
                {
                    assumptionColumn.Item().PaddingTop(5).Text(text =>
                    {
                        ...
                        {
                            text.Span($" ({confidenceLabel} {assumption.ConfidenceLevel})").Italic().FontSize(9);
                        }
                    });
                }
            });
    ...
    private void ComposeGaps(IContainer container, List<GapRecord> gaps, string? language = null)
    {
        container.Column(column =>
        {
            column.Item().Background(Colors.Red.Lighten4).Padding(8).Column(gapColumn =>
            {
                ...
                gapColumn.Item().Text(gapsLabel).FontSize(12).SemiBold();
                ...
                foreach (var gap in gaps)
                {
                    gapColumn.Item().PaddingTop(5).Text(text =>
                    {
                        ...
                        {
                            text.Span($" ({reasonLabel} {gap.MissingReason})").Italic().FontSize(9);
                        }
                    });
                }
            });
